...
// 텍스트 줄간격 · 여백 높이 · 구분선 굵기/색 · 콜아웃 배경 — admin(ContentPreview)과 같은 값으로 유지.
    ...
    case 'image':
      return (
        ...
        >
          ...
          {block.caption && (
            <figcaption className="mt-2 text-center text-[13px] text-ink/55">{renderInline(block.caption)}</figcaption>
          )}
        </figure>
      );
    case 'gallery':
    ...
    case 'callout':
      return (
        <div key={key} className={`my-4 flex gap-3 rounded-lg border p-4 ${CALLOUT_BOX[block.color ?? 'yellow']}`}>
          ...
          <div className="min-w-0 flex-1 text-[15px] leading-[1.7] text-ink/85 whitespace-pre-wrap">
            {renderInline(block.markdown)}
          </div>
        </div>
      );
