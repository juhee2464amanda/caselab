...
function devTrendBody(o: {
  ...
  return {
    kind: 'trend',
    whats_new: blocks(o.whatsNew),
    experiment: blocks(o.experiment),
    verdict: { useful: blocks(o.useful), notUseful: blocks(o.notUseful) },
  };
}
...
export const caseSeed: ContentRow[] = [
  ...
  // ── Trends (track:'trend', 발행일 분산: 1/3/6개월 필터 데모용) ──
  {
    ...
    body: devTrendBody({
      whatsNew: [
        'Claude에 이전 대화의 맥락을 기억하는 메모리 기능이 추가됐다. 핵심은 "매번 같은 배경을 다시 설명하지 않아도 된다"는 것. 내 직무, 자주 쓰는 형식, 진행 중인 프로젝트를 한 번 알려주면 다음 대화부터 그 맥락 위에서 답한다.',
        '기존에도 "프로젝트 지식"이나 커스텀 인스트럭션으로 비슷한 걸 흉내 낼 수는 있었다. 다른 점은, 이번엔 사용자가 따로 정리해 넣지 않아도 대화 과정에서 알아서 기억할 거리를 추려 저장한다는 점이다.',
      ],
      experiment: [
        '1주일간 세 가지 루틴에 메모리를 켜고 붙여봤다. (1) 매일 회의록을 같은 포맷으로 정리, (2) "지난번 그 톤으로 다시 써줘" 같은 반복 요청, (3) 진행 중인 한 프로젝트에 대한 후속 질문.',
        '첫날엔 큰 차이를 못 느꼈지만, 사흘쯤 지나자 "이 프로젝트는 B2B SaaS고, 톤은 격식체"라는 걸 다시 안 적어도 되는 게 체감되기 시작했다. 특히 회의록 포맷처럼 반복되는 작업에서 입력이 눈에 띄게 짧아졌다.',
      ],
      useful: [
        '반복되는 프로젝트 맥락을 매번 설명하지 않아도 되는 점이 가장 컸다. 같은 형식의 작업을 이어갈 때 첫 문장만 던져도 바로 내가 원하던 결과에 가깝게 나왔다.',
        '"내 직무·톤"을 기억하니 결과물의 톤 일관성도 좋아졌다. 매번 톤을 교정하던 후작업이 줄었다.',
      ],
      notUseful: [
        '서로 다른 프로젝트 맥락이 섞이면 엉뚱한 과거 대화를 끌어와 오히려 헷갈릴 때가 있었다. A 프로젝트 톤으로 B 글을 써버리는 식.',
        '중요하거나 톤이 다른 작업은 새 대화로 분리하는 게 안전했다. "무엇을 기억하고 있는지" 가끔 확인·정리해주지 않으면 오래된 가정이 답을 오염시켰다.',
      ],
    }),
    job_tags: ['planning'],
    persona_coverage: ['A'],
  ...
  },
  {
    ...
    body: devTrendBody({
      whatsNew: [
        'Gemini와 GPT 모두 최근 큰 업데이트가 있었다. "둘 중 뭐가 더 좋냐"는 질문이 많지만, 한 달 써본 결론은 "작업 종류에 따라 다르다"였다.',
        '그래서 승패를 가리기보다, 같은 업무를 두 모델에 번갈아 맡기며 "어떤 작업에 어느 쪽이 손이 덜 가는지"를 기록하는 쪽으로 실험을 잡았다.',
      ],
      experiment: [
        '한 달간 리서치·초안 작성·교정·요약 네 가지 작업에 동일한 프롬프트를 양쪽에 넣고, 결과를 그대로 붙여 비교했다. 평가 기준은 단순하게 "후작업이 얼마나 필요한가"로 통일했다.',
        '주관을 줄이려고, 어떤 모델 결과인지 모르게 섞어두고 나중에 라벨을 확인하는 식으로도 몇 번 돌려봤다.',
      ],
      useful: [
        '출처가 필요한 리서치엔 Gemini가 잘 맞았다. 근거 링크를 함께 주니 사실 확인 시간이 짧았다.',
        '빠른 초안·브레인스토밍엔 GPT가 손이 덜 갔다. 톤을 잡아 던지면 바로 쓸 만한 1차 결과가 나왔다. 작업 성격에 따라 갈라 쓰니 전체 결과 품질이 올라갔다.',
      ],
      notUseful: [
        '한 모델만 고집하면 작업마다 강점을 못 살린다. "리서치도 GPT로" 밀어붙였을 때 출처 확인에 더 오래 걸렸다.',
        '둘을 오가는 번거로움은 감수해야 한다. 구독을 둘 다 유지할지는 본인 작업 비중을 보고 정할 일.',
      ],
    }),
    job_tags: ['marketing'],
    persona_coverage: ['E'],
  ...
  },
  {
    ...
    body: devTrendBody({
      whatsNew: [
        'AI 앱과 외부 도구를 잇는 MCP(Model Context Protocol) 표준이 Claude·ChatGPT·Cursor 등 주요 클라이언트에 빠르게 확산됐다. 쉽게 말해 "AI가 내 노션·구글드라이브·캘린더 같은 도구를 직접 읽고 쓸 수 있게 연결하는 공통 규격"이다.',
        '핵심은 "한 번 만든 연결을 여러 AI 앱에서 재사용"한다는 점. 예전엔 도구마다·앱마다 따로 붙여야 했다면, MCP는 USB-C처럼 한 번 꽂으면 어디서든 통한다.',
      ],
      experiment: [
        '개발자가 아닌 실무자 관점에서, 한 달간 노션·구글드라이브 MCP 연결을 붙여보며 일상 업무가 실제로 바뀌는지 지켜봤다. 설정은 가이드를 따라 한 번 해두고, 이후엔 평소 쓰던 질문을 그대로 던졌다.',
        '"이번 주 회의 노트 요약해줘", "그 폴더의 제안서 초안 찾아서 톤 다듬어줘" 같은, 평소엔 복붙이 필요하던 요청을 연결 상태에서 시켜봤다.',
      ],
      useful: [
        '한 번 연결해두면 여러 AI 앱에서 같은 도구·데이터를 재사용할 수 있다. 파일을 매번 복붙해 넣던 반복 작업이 확실히 줄었다.',
        '"내 자료를 근거로" 답하게 만들 수 있어, 일반론 대신 우리 맥락에 맞는 결과가 나왔다. 자료 검색→정리 동선이 짧아졌다.',
      ],
      notUseful: [
        '아직 초기 설정 진입장벽이 있다. 첫 연결은 가이드 없이 비개발자가 혼자 하기엔 헷갈리는 단계가 몇 개 있었다.',
        '권한·보안을 잘 모르고 넓게 열면 위험할 수 있다. 어떤 도구에 어디까지 접근을 허용했는지 한 번은 점검하고 쓰는 게 안전하다.',
      ],
    }),
    job_tags: ['planning', 'marketing'],
    persona_coverage: ['A', 'E'],
