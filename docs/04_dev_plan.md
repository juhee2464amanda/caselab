...
### 18.18 주제 후보 받기(`/topics`) GNB 메뉴 노출 (2026-06-26)

**배경**: "이런 거 다뤄주세요" 식으로 독자에게 콘텐츠 주제 후보를 받는 기능이 필요하다는 요청. 확인 결과 `/topics` 페이지("이런 거 어때요" — 후보 목록 + 👍 투표 + 로그인 후 제안 폼)와 `topic_suggestions`/`topic_votes` 테이블·RLS가 **이미 0001/0010에서 구현·prod 배포**돼 있었음. 빠진 건 GNB 진입로뿐.

**결정**: 신규 개발 없이 **GNB 메뉴에 진입로만 추가**. MVP에서 빼지 않고 그대로 노출(빈 상태 안내 "첫 후보를 곧 올릴게요"가 있어 오픈 직후에도 자연스러움).

| # | 결정 | 영향 |
|---|---|---|
| 1 | `NAV`에 `{ label: '이런 거 어때요', href: '/topics' }` 추가 (ebook 다음) | `components/layout/GNB.tsx`. `NAV`가 PC·모바일 단일 소스라 양쪽 동시 반영 |
