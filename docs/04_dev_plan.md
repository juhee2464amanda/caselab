...
**미포함 (다음 단계)**: 비밀번호 찾기/재설정 — 재설정 메일 발송용 Supabase 커스텀 SMTP(Brevo) 연결 필요해 별도 작업. "아이디 찾기"는 ID=이메일이라 불필요.

**영향받는 파일**: `app/(public)/signup/page.tsx`(신규), `app/(public)/login/page.tsx`, `app/(public)/onboarding/page.tsx`, `components/layout/Footer.tsx`, `supabase/migrations/0013_signup_consent.sql`.
